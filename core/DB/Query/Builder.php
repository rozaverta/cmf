...
use EApp\DB\Query\Grammars\Grammar;
...
class Builder
{
	 ...
	 *
	 * @param  \Closure|\EApp\DB\Query\Builder|string $query
	 * @param  string  $as
	 * @return \EApp\DB\Query\Builder|static
	 *
	...
	public function fromSub($query, $as)
	{
		...
		$this->from = new Expression('('. $query .') as '.$this->grammar->wrap($as));
		$this->setBindings($bindings, 'from');
	 ...
	 *
	 * @param  string   $column
	 * @param  \EApp\DB\Query\Builder|static  $query
	 * @param  string   $boolean
	 * @param  bool     $not
	 * @return $this
	 */
	...
	public function groupBy(...$groups)
	{
		foreach ($groups as $group)
		{
			$this->groups = array_merge(
				(array) $this->groups, Arr::wrap($group)
			);
		}
	 ...
	 *
	 * @param  \EApp\DB\Query\Builder|\Closure  $query
	 * @param  bool  $all
	 * @return \EApp\DB\Query\Builder|static
	 */
	...
	{
		return new Collection($this->getRaw($columns));
	}
	 ...
	 * @return array
	 */
	protected function getRaw($columns = ['*'])
	{
	...
	{
		return count($raw = $this->limit(1)->getRaw($columns)) ? $raw[0] : null;
	}
	 ...
	 * @return int
	 */
	public function getCountForPagination($column = '*')
	{
		$query = $this
			->cloneWithout(['columns', 'orders', 'limit', 'offset'])
			->cloneWithoutBindings(['select', 'order']);

		$columns = $this->withoutSelectAliases([$column]);
		...
		if(isset($this->groups))
		{
			...
			$query = $this
				->newQuery()
				->fromSub( $query->select($columns), 'table_count' );
		...
		if(is_object($raw))
		{
			return (int) $raw->aggregate;
		}
		else
		{
			return (int) array_change_key_case((array) $raw)['aggregate'];
		}
	}
	 ...
	 * @throws \InvalidArgumentException
	 */
	public function setBinding($number, $value, $type = 'where')
	{
		...
		$number = (int) $number;
		if( !isset($this->bindings[$type][$number]) )
		{
			throw new InvalidArgumentException("Invalid binding number parameter: {$number}.");
		}

		$this->bindings[$type][$number] = $value;
		return $this;
	}
	...
	{
		return $this->cloneWith(function ($clone) use ($except) {
	...
	protected function cloneWith(Closure $callback)
	{
		$clone = clone $this;
		$callback($clone);
		return $clone;
