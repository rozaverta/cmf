...
class Init extends AbstractScript
{
	public function run()
	{
		...
		if( $io->confirm("Init Elastic-CMF (y/n)? ") )
		{
			$this->create();
		}
	}
	...
	{
		$io = $this->getIO();
		if( !$io->confirm("Add index and hosts files (y/n)? ") )
		{
			return;
		}
		...
		if($io->confirm("Create default host (y/n)? "))
		{
		...
		if( $apache )
		{
			$htc_file = !
				...
				$io->confirm(".htaccess file not found. Create (y/n)? ");
		}
...
');
	...
	private function askFileName($name)
	{
		...
		if( file_exists($file) )
		{
			$overwrite = $this
				->getIO()
				->confirm("The specified file already exists. Overwrite it (y/n)? ");
